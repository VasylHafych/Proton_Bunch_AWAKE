function likelihood_cam4(
    ...
    @inbounds σ_1::VT = sqrt.(params.tr_size[1]^2 + 10^-4*params.ang_spr[1]^2*(params.waist[1] - params.s_cam[cam_ind])^2) 
    
    σ_x_1_res::VT = sqrt(σ_1^2 + (params.cam4_resx*δ_x)^2)
    σ_y_1_res::VT = sqrt(σ_1^2 + (params.cam4_resy*δ_y)^2) 
    
    @inbounds σ_2::VT = sqrt.(params.tr_size_2[1]^2 + 10^-4*params.ang_spr_2[1]^2*(params.waist_2[1] - params.s_cam[cam_ind])^2) 
    
    σ_x_2_res::VT = sqrt(σ_2^2 + (params.cam4_resx*δ_x)^2)
    σ_y_2_res::VT = sqrt(σ_2^2 + (params.cam4_resy*δ_y)^2) 
...
function likelihood_cam13(
    ...
    σ_x_1_res::VT = sqrt(σ_1^2 + (params.resx[cam_ind]*δ_x)^2)
    σ_y_1_res::VT = sqrt(σ_1^2 + (params.resy[cam_ind]*δ_y)^2) 
    
    @inbounds σ_2::VT = sqrt.(params.tr_size_2[1]^2 + 10^-4*params.ang_spr_2[1]^2*(params.waist_2[1] - params.s_cam[cam_ind])^2) 
    
    σ_x_2_res::VT = sqrt(σ_2^2 + (params.resx[cam_ind]*δ_x)^2)
    σ_y_2_res::VT = sqrt(σ_2^2 + (params.resy[cam_ind]*δ_y)^2) 
...
function generate_image_cam13(
    ...
    @inbounds σ_1 = sqrt.(params.tr_size[1]^2 + 10^-4*params.ang_spr[1]^2*(params.waist[1] - params.s_cam[cam_ind])^2) 
    
    σ_x_1_res = sqrt(σ_1^2 + (params.resx[cam_ind]*δ_x)^2)
    σ_y_1_res = sqrt(σ_1^2 + (params.resy[cam_ind]*δ_y)^2) 
    
    @inbounds σ_2 = sqrt.(params.tr_size_2[1]^2 + 10^-4*params.ang_spr_2[1]^2*(params.waist_2[1] - params.s_cam[cam_ind])^2)  
    
    σ_x_2_res = sqrt(σ_2^2 + (params.resx[cam_ind]*δ_x)^2)
    σ_y_2_res = sqrt(σ_2^2 + (params.resy[cam_ind]*δ_y)^2) 
...
function generate_image_cam4(
    ...
    σ_x_1_res = sqrt(σ_1^2 + (params.cam4_resx*δ_x)^2)
    σ_y_1_res = sqrt(σ_1^2 + (params.cam4_resy*δ_y)^2) 
    
    @inbounds σ_2 = sqrt.(params.tr_size_2[1]^2 + 10^-4*params.ang_spr_2[1]^2*(params.waist_2[1] - params.s_cam[cam_ind])^2) 
    
    σ_x_2_res = sqrt(σ_2^2 + (params.cam4_resx*δ_x)^2)
    σ_y_2_res = sqrt(σ_2^2 + (params.cam4_resy*δ_y)^2) 
