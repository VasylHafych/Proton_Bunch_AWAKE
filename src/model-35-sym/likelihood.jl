...
function generate_image_cam13(
    ...
    σ::Float64 = sqrt.(params.tr_size[1]^2 + 10^-4*params.ang_spr[1]^2*(params.waist[1] - params.s_cam[cam_ind])^2) 
    
    σ_x = sqrt(σ^2 + (params.resx[cam_ind]*δ_x).^2)
    σ_y = sqrt(σ^2 + (params.resy[cam_ind]*δ_y).^2)
...
function generate_image_cam4(
    ...
    σ_x = sqrt(σ^2 + (params.cam4_resx*δ_x)^2)
    σ_y = sqrt(σ^2 + (params.cam4_resy*δ_y)^2)
...
function likelihood_cam4(
    ...
    σ_x::VT = sqrt(σ^2 + (params.cam4_resx*δ_x)^2)
    σ_y::VT = sqrt(σ^2 + (params.cam4_resy*δ_y)^2)
...
#                 @inbounds cum_log_lik += logpdf(truncated(Normal(pix_prediction, params.cam4_light_fluct*sqrt(pix_prediction)), 0.0, 4096.0), image[pix_ind]) # leads to -Inf in some cases
...
function likelihood_cam13(
    ...
    σ_x::VT = sqrt(σ^2 + (params.resx[cam_ind]*δ_x)^2)
    σ_y::VT = sqrt(σ^2 + (params.resy[cam_ind]*δ_y)^2) 
